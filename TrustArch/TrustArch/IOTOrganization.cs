...
namespace TrustArch
{
    ...
    public class IOTOrganization:Organization
    {
        ...
        public void ReceiveGetMonitorResponse(Packet pkg)
        {
            GetMonitorResponseField resp = pkg.GetMonitorResponse;
            int monitorNode = resp.monitorNode;
            int monitorOrg = resp.monitorOrg;
            int networkId = resp.network;

            if (monitorNode < 0)//if no trust node still not exist
            {
                Console.WriteLine("Cannot find a trust node for network {0}", networkId);
                return;
            }
            //获取间接信任
            Dictionary<int, IOTOrganizationTrust> orgTrust = global.trustManager.getOrgReputations();
            double indirectTrust = orgTrust[monitorOrg].trustValue;
            //计算直接信任
            double directTrustRatio, directTrust;
            if (this.cachedOrgTrustResult.ContainsKey(monitorOrg))
            {
                directTrustRatio = 0.6;
                directTrust = this.cachedOrgTrustResult[monitorOrg].trustValue;
            }
            else
            {
                directTrustRatio = 0;
            ...
            //获得最终的信任，如果信任值不够，则拒绝该授权请求
            double trust = directTrustRatio * directTrust + (1 - directTrustRatio) * indirectTrust;
            if ( trust < global.TrustThrehold)
            {
                Console.WriteLine("Recommended monitor{0} of org{1} is untrustable", monitorNode, monitorOrg);
                return;
            }
            if (!this.cachedMonitorRequests.ContainsKey(networkId))
                return;
            ...
            //取出该网络中所有节点的所有标签请求
            Dictionary<int, HashSet<int>> temp = this.cachedMonitorRequests[networkId];
            foreach(KeyValuePair<int, HashSet<int>> k in temp)
            {
                int node = k.Key;
                int[] tags = k.Value.ToArray();
                int[] keys = new int[tags.Length];
                tags.CopyTo(keys, 0);

                //send auth key to the reader..
                Packet pkg1 = new Packet(this, global.readers[node], PacketType.AUTHORIZATION);
                pkg1.Next = pkg.Prev;
                pkg1.NextType = pkg.PrevType;
                //TODO auth key should be made.
            ...
            this.cachedMonitorRequests.Remove(networkId);
            if (!this.trustNodes.ContainsKey(networkId))
                this.trustNodes.Add(networkId, new List<int>());
            this.trustNodes[networkId].Add(monitorNode);

            //make a monitor..
            Packet pkg2 = new Packet(this, global.readers[monitorNode], PacketType.SET_MONITOR);
            pkg2.Next = pkg.Prev;
            pkg2.NextType = pkg.PrevType;
            pkg2.SetMonitorResponse = new SetMonitorResponseField(monitorNode, this.Id, networkId);
        ...
        //每隔一段时间检查一下节点的信任值
        override public void CheckRoutine()
        {
