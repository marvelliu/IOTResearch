    ...
    public partial class MainForm : BaseForm
    {
        ...
        void DrawNodes(Graphics gc)
        {
            ...
            for (int i = 0; i < global.objectNum; i++)
            {
                ...
                ObjectNode o = global.objects[i];

                gc.FillEllipse(brush, (float)o.X - r / 2 + offsetX,
                        (float)o.Y - r / 2 + offsetY, r, r);
                gc.DrawString("O" + o.Id.ToString(), new Font("arial", 10), brush,
                    (float)o.X + offsetX, (float)o.Y + offsetY);

                brush = new SolidBrush(Color.DarkBlue);
                pen = new Pen(brush);
                Reader node = o.lastNearReader;
                if(node !=null)
                    gc.DrawLine(pen, (float)o.X + offsetX,
                        (float)o.Y + offsetY,
